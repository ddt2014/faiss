...
int main() {
  ...
  faiss::gpu::GpuIndexFlat index_ivf(&res, d, faiss::METRIC_L2);
  ...
  {     // search xq
    ...
    for(int i = 0; i < nq; i++) {
/*      gettimeofday(&begin,NULL);
      index_ivf.search(1, x_query + cnt, k, D + cnt2, I + cnt2);
      gettimeofday(&end,NULL);
      diff = (end.tv_sec - begin.tv_sec) * 1000.0 + (end.tv_usec - begin.tv_usec) / 1000.0;
      sum_time += diff;
      cout << "itme: " << i << " search time: " << diff << " ms, ave: "  << sum_time / (i + 1) << " ms, number: " << i << endl;
*/
      json::JSON query;
